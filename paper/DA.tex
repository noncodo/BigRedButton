...
Understanding themolecular mechanisms underlying the biological functions of lncRNAs 
...
by \carna{} \cite{Palu2010,Sorescu2012}, which iteratively improves alignments 
using a constraint programming technique implementing a branch and bound scheme. 
Another heuristic is pruning of the dynamical programming matrix without pre-folding 
constraints, as implemented by \texttt{FoldAlign} \cite{Havgaard17937495,Sundfeld26704597}. \\ 
...
base pairing probabilities for each queried sequence. 
...
of \dotaligner{} can be found in Additional file 1.\\
...
where any two sequences from the same family share between 0-55\% and 56-95\% pairwise sequence identity, respectively. The \texttt{JAVA} implementation of this algorithm, derived from \cite{smith2013widespread}, can be found in the Additional file 1. 
...
\begin{enumerate}
  \item Pairwise probabilistic string alignment; 
  \item Sampling of stochastic string alignments and scoring of aligned dot plots.
\end{enumerate}

\subsubsection*{1.1 Pairwise probabilistic string alignment}
...
\subsubsection*{1.2 Sampling of stochastic string alignments and scoring of aligned dot plots}
...
\begin{figure}
 ...
 \caption*{ \textbf{ Figure S4. Impact of sampling of stochastic alignments on combined score.}\\
 We executed \dotaligner{} (default parameters) while varying parameter T (sampling diversity) with 1000 samples (parameter s) on the RFAM binary classification benchmark datasets corresponding to 0-55\% and 56-65\% sequence identity (SI) (3 replicates each). For parameter T equal 0.1 and 0.25 the majority of pairwise alignments are optimized through the sampling procedure (sample increased the combined score). In few cases, T=0.5 also produced an optimized alignment through sampling. 
}
\end{figure}

\begin{figure}
 \includegraphics[width=0.45\textwidth]{SF5}
 \caption*{ \textbf{ Figure S5. Pairwise alignments of dot plots of SAM riboswitch. }\\
