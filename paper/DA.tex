...
sequences of interest.  Our approach outperforms other tools at classifying known RNA structure families, both in time and accuracy. It identifies clusters of known and novel structure motifs from ENCODE immunoprecipitation data of 44 RNA-binding proteins. 
\end{abstract}
...
In contrast, the heuristics implemented in \texttt{RNApaln} increased the speed of 
...
To test whether this is due to local stretches of sequence similarity, we re-aligned RFAM 
sequences with a variant of the NW algorithm permitting free end gaps. 
Unsurprisingly, this increased the overall average pairwise sequence similarity of alignments 
from 59\% to 72\%, with a notable drop in alignments displaying less than 50\% pairwise 
sequence  identity from >21\% to <0.4\% (from . 
This suggests that most representatives of  the RFAM seed alignments present local stretches 
of sequence similarity, which substantiates the observed performance of the NW algorithm.\\
}
...
accuracy and speed based on a collection of RFAM seed alignments, and show that these default settings are applicable on de-novo data from an eCLIP experiment. \\
...
proteins. \textcolor{red}{When compared to  \locarna{} with default parameters,}
 \dotaligner{} calculates semi-local alignments by introducing penalty-free gaps at the sequence extremities. 
...
were filtered to keep only those with $\ge$8x fold enrichment over the total input background and an associated P-value  $\leq10^{-4}$. Furthermore, peaks were merged if they overlapped by more than 50 nt to avoid over-representing the same sequence (Additional file 1). The remaining peaks were subsequently filtered by 
...
\begin{figure}[h!]
 ...
 (\textbf{B}) Reachability plot displaying the OPTICS-derived ordering of points (x axis) and their distance to the nearest neighbour (y axis) of 2,650 ENCODE eCLIP peaks + 100 RFAM controls overlapping evolutionarily conserved secondary structure predictions. The dominant RNA binding protein  in each cluster is displayed next to significant clusters; those with an asterisk and portrayed below.
...
An unannotated DKC1-bound sequence is marked with an a=sterix.
