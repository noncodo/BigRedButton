...
/*---------------------------------------------------------------------------*/
      /*------------------------ alignment functions -------------------*/
/*---------------------------------------------------------------------------*/
/* this function converts an alignment encoded as a character srting to an
...
al decode_alig(aligm ai)
{
  ...
  /* lenght of the trace (alignment encodes in 1,2,3,4) array */
  j=strlen(ai.a);
  la=j;
...
/*---------------------------------------------------------------------------*/
/*---------------------------------------------------------------------------*/
/* decides wheter a all plus matrix or a +/- matrix is used */
int plus(void)
{
  int i, swi, lm;
...
/* calculate the score for an optimal alignement using the gotoh algotithm */
...
/*---------------------------------------------------------------------------*/
  /* calculate the partitionfunction using a modification of the gotoh
     algorithmen for sequence alignement */
...
aligm stoch_btr(matrix pz, aligm a)
{
  ...
  pZ=pz.M[0][0];/* partion fuction over all aligments */ 
